...
import { MDBTable, MDBTableHead, MDBTableBody, MDBBadge, MDBBtn, MDBCol, MDBRow, MDBTooltip, MDBInputGroup, MDBNavbar, MDBCard, MDBCardBody, MDBBtnGroup, MDBContainer, MDBAccordion, MDBAccordionItem } from 'mdb-react-ui-kit'
  ...
  return (
    <>
      ...
      <div className={styles.tableFixHead}>
        ...
        <MDBTable color='dark' borderless responsive hover={view !== 'card'} className='text-break mx-0' style={{ tableLayout: 'fixed' }}>
            ...
            {chamadosFiltrados.map((chamado: IChamado) => {
              ...
              if (view === 'card') return (
                <tr key={`${chamado.Cliente.Id}#${chamado.Id}`}>
                  <td colSpan={11}>
                    <MDBRow className={`justify-content-start rounded mx-2 py-3  shadow-sm ${styles.cardViewRow}`} style={{ rowGap: 20 }}>

                      <MDBCol size={12} md={1} className='d-flex align-items-center text-center flex-row justify-content-evenly'>
                      ...
                      <MDBCol size={12} md={5} className='d-flex align-items-center' style={{ columnGap: 20 }}>

                        <MDBCol size={6} style={{ maxWidth: '150px' }}>
                          <IdElm chamado={chamado} />
                        </MDBCol>

                        <MDBCol size={1} className={`${styles.col_Bug} ${comBug ? '' : 'd-none'}`}>
                      ...
                      <MDBCol size={12} md={6} className='d-flex align-items-center text-center flex-row justify-content-evenly'>
                        ...
                        <MDBCol size={4}>
                          ...
                          <MDBRow>
                            <AtribuidaElm chamado={chamado} />
                          </MDBRow>
                        </MDBCol>

                        <MDBCol size={4}>
                          <MDBRow>
                            <TipoSolicitacaoElm chamado={chamado} />
                          </MDBRow>
                          <MDBRow>
                            <EmailClienteElm chamado={chamado} className='fst-italic fw-lighter' />
                          </MDBRow>
                        </MDBCol>
                      ...
                      <MDBCol size={12}>
                        <MDBAccordion className='bg-transparent'>
                          <MDBAccordionItem collapseId={1} headerClassName='rounded bg-transparent' bodyClassName='rounded bg-transparent' headerTitle='' className='rounded bg-dark'>
                            <MDBRow>
                              <MDBCol size={12} md={6}>
                                <span className='fs-5 text-light'>Descrição:</span>
                                <hr />
                                <DescricaoDemandaElm chamado={chamado} />
                              </MDBCol>
                              <MDBCol size={12} md={6}>
                                <span className='fs-5 text-light'>Comentários:</span>
                                <hr />
                                <ComentariosElm chamado={chamado} />
                              </MDBCol>
                            </MDBRow>
                          </MDBAccordionItem>
                        </MDBAccordion>
...
const DescricaoDemandaElm = (pr: IElmProps) => (
  <div
    className={styles.divDescricaoDemanda}
    dangerouslySetInnerHTML={{ '__html': `${pr.chamado.DescricaoDemanda?.trim()}` }}
  />
)
const ComentariosElm = (pr: IElmProps) => (
  <span className={styles.divDescricaoDemanda}>
    {pr.chamado.Comentarios?.trim()}
  </span>
)
...
const TipoSolicitacaoElm = (pr: IElmProps) => (
  <span className={pr.className}>{pr.chamado.TipoSolicitacao}</span>
)
const EmailClienteElm = (pr: IElmProps) => (
  <small className={pr.className}>{pr.chamado.EmailCliente}</small>
)
const AtribuidaElm = (pr: IElmProps) => {

  const colorStatus = fieldStatus(pr.chamado).Atribuida
  const color = colorStatus === 'success' ? 'light' : colorStatus
